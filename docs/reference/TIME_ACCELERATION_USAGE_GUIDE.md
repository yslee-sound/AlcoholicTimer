...
```
┌──────────────────────┐
│   Day 416 / 30일      │  ← 1시간만에 416일 달성 (10000배속)
│   [████████████] 100% │
│                       │
│   10,000:00:00 ⚡    │  ← 중앙 타이머도 배속 반영!
│   416.67일 경과       │  ← 실시간 업데이트
└──────────────────────┘
```

> **💡 중앙 타이머 동작 (배속 반영)**:
> - **정상 모드 (1배속)**: 실제 1시간 경과 → 타이머 "01:00:00" 표시
> - **60배속**: 실제 1시간 경과 → 타이머 "60:00:00" (2.5일) 표시
> - **100배속**: 실제 1시간 경과 → 타이머 "100:00:00" (4.17일) 표시
> - **1000배속**: 실제 1시간 경과 → 타이머 "1,000:00:00" (41.67일) 표시
> - **10000배속**: 실제 1시간 경과 → 타이머 "10,000:00:00" (416.67일) 표시 ⚡
> 
> **배속 동작 원리**:
> - 실제 경과 시간은 그대로, "1일의 길이"만 짧아짐
> - **60배속**: 1일 = 24분 → 실제 1시간 경과 = 2.5일 표시
> - **100배속**: 1일 = 14.4분 → 실제 1시간 경과 = 4.17일 표시
> - **1000배속**: 1일 = 86.4초 → 실제 1시간 경과 = 41.67일 표시
> - **10000배속**: 1일 = 8.64초 → 실제 1시간 경과 = 416.67일 표시
