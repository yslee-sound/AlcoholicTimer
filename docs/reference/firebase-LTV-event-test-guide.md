네, 알겠습니다. 고객님께서 제공해주신 \*\*최신 버전의 검증 가이드(시간 변경 테스트 포함)\*\*를 바탕으로, 누락되었던 **'앱 오프닝 광고(App Open Ad)'** 검증 항목을 추가하여 **최종 완성된 문서**를 작성해 드립니다.

이 문서는 개발 에이전트에게 전달하여 **모든 핵심 이벤트와 수익화 모델**을 빈틈없이 검증하는 데 사용됩니다.

-----

# 📝 Firebase 이벤트 최종 검증 테스트 가이드 (완성본)
...
| **2. 금주 목표 달성** | **짧은 시간의 목표를 새로 설정**하고 완료될 때까지 기다리세요. (또는 시간 변경 사용) | `timer_finish` | `target_days` (금주 목표일) |
| **3. 금주 중도 포기** | 진행 중인 금주 목표를 **'종료' 버튼을 길게 눌러** 중도에 포기하세요. | `timer_end` | (없음) |
| **4. 기록 화면 진입** | 하단 탭에서 **기록** 메뉴로 이동하여 과거 기록 목록을 확인하세요. | `view_records` | (없음) |
| **5. 통계 뷰 변경** | 기록 화면에서 **'월' 통계에서 '년' 통계로** 스위치를 변경하세요. | `change_record_view` | `view_type` (예: "Year"), `current_level` |
| **6. 기록 상세 조회** | 기록 목록에서 **특정 기록 카드**를 눌러 세부 내용을 확인하세요. | `view_record_detail` | `record_id` |
| **7. 앱 오프닝 광고 (중요)** | 앱을 **완전히 종료(강제 종료)** 했다가 아이콘을 눌러 **다시 실행**하세요. 로고 화면 후 광고가 떠야 합니다. | `ad_impression`<br>`ad_revenue` | `ad_type` (값: **"app\_open"**)<br>`value` (수익 금액), `currency` |
| **8. 전면 광고 노출** | 앱을 사용하다 보면 **전면 광고**가 표시됩니다. 광고 화면이 나타나는 것을 확인하세요. | `ad_impression`<br>`ad_revenue` | `ad_type` (값: **"interstitial"**)<br>`value` (수익 금액), `currency` |
| **9. 광고 클릭** | 테스트용 광고를 **클릭**하여 브라우저로 이동하는 것을 확인하세요. | `ad_click` | `ad_type` |

**참고**: `ad_click` 이벤트는 테스트 환경에서 실제 광고주에게 비용이 발생할 수 있으니, 검증 후에는 클릭을 자제해 주십시오.

-----

모든 테스트 단계에서 Logcat과 DebugView에 해당 이벤트 이름과 \*\*필수 매개변수(특히 ad\_type 구분)\*\*가 나타나면, 모든 이벤트가 성공적으로 구현된 것입니다.