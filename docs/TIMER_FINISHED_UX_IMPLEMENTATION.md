...
- **[NEW] 헤더 구조**: Row로 변경하여 제목과 버튼 배치
  - 좌측: "최근 금주 일기" (titleMedium, Bold, onSurface)
  - 우측: 전체 보기 버튼 (우측 화살표 아이콘)
- 제목과 카드 간격: 12dp
- 흰색 배경 카드 (RoundedCornerShape 12dp)
- 일기 항목 리스트 (최대 3개 표시)

**전체 보기 버튼**:
- 아이콘: `ic_caret_right` (20dp)
- 색상: onSurfaceVariant (회색 계열)
- 클릭: TODO - 전체 일기 목록 화면으로 이동

**글쓰기 FAB (Floating Action Button)**:
- 위치: 화면 우측 하단 (16dp 여백)
- 아이콘: `ic_plus` (24dp, 연필/작성 아이콘)
- 배경색: MaterialTheme.colorScheme.primary
- 전경색: onPrimary (흰색)
- 기능: 일기 작성 화면으로 이동 (`onAddRecord` 콜백)
...
## ✍️ 일기 작성 화면 구현 (2025-01-02 추가)

### 개요

Tab02 화면의 FAB(+ 버튼)을 누르면 나타나는 **일기 작성 화면**이 구현되었습니다.
"감정 터치 & 퀵 메모" 컨셉으로 설계되어 사용자가 빠르게 기록하고 수익화(광고)로 연결됩니다.

### UI 구성

```
┌─────────────────────────────────────┐
│  ←            일기 쓰기       저장   │ ← [수정] 공통 BackTopBar
├─────────────────────────────────────┤
│  📅 2025년 12월 2일 (월) 오후 6:30  │ ← 날짜/시간
├─────────────────────────────────────┤
│  오늘 하루, 어떠셨나요? (필수)       │
│                                     │
│  😊   😌   😢   😡   😰           │ ← 기분 선택
...
#### 1. 날짜/시간 영역
- 현재 날짜와 시간 자동 표시
- 한국어 포맷: "yyyy년 M월 d일 (E) a h:mm"
- 흰색 카드 배경, 둥근 모서리

#### 2. 기분 선택 (필수)
- **5가지 기분**: 뿌듯(😊), 평온(😌), 우울(😢), 화남(😡), 갈망(😰)
- 원형 버튼, 선택 시 색상 강조
- 필수 항목: 선택해야 저장 버튼 활성화
...
#### 4. 텍스트 입력 (선택)
- 최소 높이: 150dp
- Placeholder: 질문 형태로 작성 유도
- 자유로운 메모 입력

#### 5. 저장 버튼
- 하단 고정, 눈에 띄는 Primary 색상
- 기분 미선택 시 비활성화
- 클릭 시: **저장 → 광고 정책 체크 → 전면 광고 → 완료**
...
[광고 정책 체크]
    ↓
[광고 가능?]
    ├─ YES → [전면 광고 노출] → [광고 종료] → [Records 화면 복귀]
    └─ NO  → [즉시 Records 화면 복귀]
    ↓
[일기 목록에 새 항목 추가됨]
```
...
| 요소 | 스타일 |
|:---|:---|
| 상단바 | 공통 BackTopBar (앱 전체 통일) |
| 뒤로가기 버튼 | 좌측 화살표 아이콘 (ic_caret_left) |
| 저장 버튼 | 우측 TextButton, Primary 색상 |
| 배경 | #F8F9FA (연한 회색) |
| 카드 배경 | White |
| 카드 모서리 | RoundedCornerShape(12.dp) |
| 카드 패딩 | 20dp |
...
| 부제목 색상 | #94A3B8 (연한 회색) |
| 저장 버튼 높이 | 56dp |
| 저장 버튼 색상 | Primary (활성화), #E0E0E0 (비활성화) |

### 구현 파일

1. **`DiaryWriteScreen.kt`** (신규)
   - `DiaryWriteScreen` - 메인 화면
   - `MoodType` - 기분 타입 enum
   - `DateTimeSection` - 날짜/시간 영역
   - `MoodSelectionSection` - 기분 선택
   - `CravingSliderSection` - 욕구 게이지
   - `DiaryTextInputSection` - 텍스트 입력

2. **`Screen.kt`**
   - `DiaryWrite` Screen 추가

3. **`NavGraph.kt`**
   - DiaryWrite 화면 route 추가
   - 저장 시 광고 로직 연결
   - RecordsScreen FAB → DiaryWrite 연결

### UX 설계 철학

1. **진입 장벽 최소화**
   - 기분 아이콘만 터치해도 기록 완료 가능
   - 글쓰기 부담 제거
