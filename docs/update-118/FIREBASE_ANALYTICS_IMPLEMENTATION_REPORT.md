...
**작업일**: 2025-12-31  
**상태**: ✅ Phase 1 완료 (4개) + Phase 2 부분 완료 (1개)  
**빌드 상태**: ✅ 성공
...
**구현 위치**:
- `CommunityViewModel.kt`: `addPost()` 함수에서 Firestore 저장 직후 호출

**파라미터**:
```kotlin
{
  "post_type": "community",  // 커뮤니티 게시글
  "has_image": Boolean,       // 이미지 포함 여부
  "content_length": Int,      // 글자 수
  "tag_type": String?,        // 태그 (thanks/reflect/cheer)
  "user_level": Int,          // 작성자 레벨
  "days": Int                 // 금주 일수
}
```

**호출 시점**: 커뮤니티에 게시글 작성 완료 시

**비즈니스 가치**:
- 커뮤니티 활성도 측정
- 레벨별 참여율 분석
- 콘텐츠 품질 분석 (이미지 포함 vs 텍스트만)
